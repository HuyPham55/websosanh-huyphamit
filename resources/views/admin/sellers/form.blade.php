    ...
    <div class="row">
        <div class="col-md-10">
            <div class="form-group">
                ...
                <?php
                $arrayCategories = array_column($post->productCategories->toArray(), 'id') ?? [];
                ?>
                <select id="category" name="categories[]" class="form-control select2" multiple>
                    @forelse($categories as $category_id => $category_title)
                        <option value="{{ $category_id }}" {{ in_array($category_id, old('categories') ?? []) || in_array($category_id, $arrayCategories) ? 'selected' : '' }}>
                            {{ $category_title }}
                        </option>
                    @empty
