    ...
    <div class="row">
        <div class="col-12">
            ...
            @can('show_list_footer_slides')
                @includeIf('components.buttons.add', ['route' => route('footer_slides.add')])
            @endcan
            <div class="card">
    ...
    <script type="text/javascript">
        jQuery(() => {
            ...
            jQuery('.change-status').on('switchChange.bootstrapSwitch', function (event) {
                ...
                if (itemId) {
                    postData("{{ route('footer_slides.change_status') }}", {
                        'field': field,
                        'item_id': itemId,
            ...
            jQuery('.update-sorting').on('input', function () {
                ...
                if (itemId) {
                    postData("{{ route('footer_slides.change_sorting') }}", {
                        'item_id': itemId,
                        'sorting': sorting,
                        '_token': '{{ csrf_token() }}'
                    });
