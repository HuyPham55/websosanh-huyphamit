                    ...
                    <div id="tab_seo_{{ $langKey }}" class="tab-pane container p-0 {{ $loop->first ? 'active' : '' }}">
                        <div class="form-group">
                            ...
                            <input type="text" name="{{ $langKey }}[seo_title]" id="{{ $langKey }}[seo_title]"
                                   value="{{ old("$langKey.title") ?? $data->seo_title }}"
                                   autocomplete="off"
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="{{ $langKey }}[seo_description]" id="{{ $langKey }}[seo_description]"
                                      class="form-control" rows="5" maxlength="300"
                            >{{ old("$langKey.description") ?? $data->seo_description }}</textarea>
                        </div>
                    </div>
                @endforeach
            </div>
