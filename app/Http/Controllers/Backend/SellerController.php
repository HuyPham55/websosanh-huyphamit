...
class SellerController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->routeList = 'sellers.list';
        $this->pathView = 'admin.sellers';
    }

    public function index()
    {
        session(['url.intended' => url()->full()]);
        return view("{$this->pathView}.list");
    }
    ...
    public function datatables(Request $request)
    {
        ...
        $data = DataTables::eloquent($posts)
            ...
            ->addColumn('action', function ($item) {
                return view('components.buttons.edit', ['route' => route('sellers.edit', ['id' => $item->id])])
                    . ' ' .
                    view('components.buttons.delete', ['route' => route('sellers.delete'), 'id' => $item->id]);
            })
